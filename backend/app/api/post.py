...
# 
@router.get("/post", response_model=PostList, status_code=201)
async def get_posts(
    response: Response,
...
async def get_taboolas(
    ...
    total = await session.scalar(select(func.count(Taboola.id)).filter(Taboola.domain_id==request_params.record_id))
